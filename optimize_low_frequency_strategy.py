#!/usr/bin/env python3
"""Snapshot + low-frequency optimizer for LIVE_SIGNAL_CONFIG.

Priorities (in order):
1) Bring trade frequency toward ~3 trades/day (acceptable band 3-5/day).
2) Keep strategy profitable with strong Sharpe.
3) Avoid excessive drawdown.

This script runs 3 rounds. Each round snapshots server.py and applies changes
only when the round objective improves under guardrails; otherwise it rolls back.
"""
...
def _estimate_macro_bias(close: list[float]) -> int:
    if len(close) < 2:
        return 0
    lookback = min(len(close) - 1, 390)
    mom = close[-1] / close[-(lookback + 1)] - 1.0
    bias = 1 if mom > 0 else -1 if mom < 0 else 0
    # If a live server is up, prefer its bias to better match runtime behavior.
    try:
        with urlopen("http://localhost:8080/api/dashboard", timeout=1.2) as resp:
            payload = json.loads(resp.read().decode("utf-8"))
            bias = int(payload.get("pdf_trend", {}).get("bias", bias))
    except Exception:
        pass
    return bias
...
def _sanitize(params: dict[str, float]) -> dict[str, float]:
    ...
    out["momentum_threshold"] = float(_clamp(float(out.get("momentum_threshold", 8e-5)), 2e-5, 4.2e-4))
    out["volatility_quantile_cap"] = float(_clamp(float(out.get("volatility_quantile_cap", 0.62)), 0.38, 0.95))
    out["countertrend_multiplier"] = float(_clamp(float(out.get("countertrend_multiplier", 2.0)), 1.0, 4.5))
    out["min_hold_bars"] = int(_clamp(float(out.get("min_hold_bars", 18)), 10, 40))
    out["macro_countertrend_allow_multiplier"] = float(
        _clamp(float(out.get("macro_countertrend_allow_multiplier", 3.0)), 1.0, 8.0)
    )
    out["trend_regime_multiplier"] = float(_clamp(float(out.get("trend_regime_multiplier", 1.2)), 0.15, 2.2))
    out["neutral_regime_multiplier"] = float(_clamp(float(out.get("neutral_regime_multiplier", 1.0)), 0.5, 4.0))
    out["high_volatility_multiplier"] = float(_clamp(float(out.get("high_volatility_multiplier", 0.9)), 0.5, 1.9))
    out["short_entry_multiplier"] = float(_clamp(float(out.get("short_entry_multiplier", 1.1)), 0.35, 2.2))
...
def _random_params() -> dict[str, float]:
    return {
        ...
        "countertrend_multiplier": random.uniform(1.0, 4.2),
        "min_hold_bars": random.randint(12, 36),
        "macro_countertrend_allow_multiplier": random.uniform(1.0, 7.2),
        "trend_regime_multiplier": random.uniform(0.2, 2.0),
        "neutral_regime_multiplier": random.uniform(0.6, 3.8),
        "high_volatility_multiplier": random.uniform(0.55, 1.8),
        "short_entry_multiplier": random.uniform(0.45, 2.0),
        "disable_longs_when_macro_short": random.choice([True, False]),
    }


def _mutate(center: dict[str, float], strength: float = 0.3) -> dict[str, float]:
    out = dict(center)
    ...
    out["momentum_threshold"] *= math.exp(random.gauss(0.0, 0.46 * strength))
    out["volatility_quantile_cap"] += random.gauss(0.0, 0.11 * strength)
    out["countertrend_multiplier"] += random.gauss(0.0, 0.55 * strength)
    out["min_hold_bars"] += int(round(random.gauss(0.0, 4.0 * strength)))
    out["macro_countertrend_allow_multiplier"] += random.gauss(0.0, 0.75 * strength)
    out["trend_regime_multiplier"] += random.gauss(0.0, 0.30 * strength)
    out["neutral_regime_multiplier"] += random.gauss(0.0, 0.50 * strength)
    out["high_volatility_multiplier"] += random.gauss(0.0, 0.20 * strength)
    out["short_entry_multiplier"] += random.gauss(0.0, 0.25 * strength)
...
def _score(metrics: dict[str, float]) -> float:
    ...
    freq_distance = abs(tpd - TARGET_TRADES_PER_DAY)
    score = 0.0
    score -= 2.9 * freq_distance
    if tpd > MAX_TRADES_PER_DAY:
        score -= (tpd - MAX_TRADES_PER_DAY) * 4.2
    if tpd < 2.3:
        score -= (2.3 - tpd) * 1.8

    score += 0.90 * metrics["total"]
    score += 0.22 * metrics["sh"]
    score += 0.22 * metrics["dd"]  # dd is negative

    if metrics["total"] < 0:
        score -= 4.0 + abs(metrics["total"]) * 1.2
    if metrics["dd"] < -2.5:
        score -= (abs(metrics["dd"]) - 2.5) * 2.4
    return float(score)
...
def main() -> None:
    ...
    if len(returns) < 300:
        raise RuntimeError("Insufficient bars for low-frequency optimization")
    bar_ts = timestamps[1:]
    macro_bias = _estimate_macro_bias(close)
    ...
    def simulate(params: dict[str, float]) -> dict[str, float]:
        ...
        days = set()
        for ts in bar_ts:
            d = datetime.fromtimestamp(ts, tz=timezone.utc).astimezone(server.ET_TZ).date()
            days.add(d)
    ...
    summary_rounds: list[dict[str, object]] = []

    for rnd in range(1, 4):
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        snapshot = SNAPSHOT_DIR / f"server.py.{stamp}.lowfreq.round{rnd}.bak"
        shutil.copy2(SERVER_FILE, snapshot)

        base = simulate(current)
        base["score"] = _score(base)
        best_p = dict(current)
        best_m = dict(base)
        ...
        # Global low-frequency exploration.
        ...
        for _ in range(7000):
            cand = _mutate(best_p, strength=0.30)
            m = eval_params(cand)
            if m["score"] > best_m["score"]:
                best_p = _sanitize(cand)
                best_m = dict(m)

        base_dist = abs(base["trades_per_day"] - TARGET_TRADES_PER_DAY)
        best_dist = abs(best_m["trades_per_day"] - TARGET_TRADES_PER_DAY)
        improved = (
            best_m["score"] > base["score"] + 0.08
            and best_m["total"] > 0.0
            and best_m["trades_per_day"] <= MAX_TRADES_PER_DAY + 0.25
            and (best_dist < base_dist - 0.12 or best_m["total"] > base["total"] + 0.25)
        )
    ...
    final_metrics["score"] = _score(final_metrics)
    summary = {
        "objective": {
            "target_trades_per_day": TARGET_TRADES_PER_DAY,
            "acceptable_trades_per_day": [3.0, 5.0],
            "priority": "frequency -> profitability -> drawdown",
        },
    ...
    out = SNAPSHOT_DIR / f"optimization_lowfreq_3rounds.{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
